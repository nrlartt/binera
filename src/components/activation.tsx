...
export function Activation({ agent }: { agent: Agent }) {
  ...
  const [open, setOpen] = useState(false); const [task, setTask] = useState(`Research ${agent.categories[0] === "health" ? "my lending position and its liquidation risks" : agent.categories[0] === "grid" ? "a grid trading strategy" : agent.categories[0] === "rebalancing" ? "liquidity position rebalancing options" : "yield opportunities"} on BNB Chain. Include sources, timestamps and risk flags.`);
  ...
  async function getQuote() { if (lock.current) return; lock.current = true; setBusy("Checking the live seller and its signed quote…"); setError(""); setQuote(null); setApproved(false); try { setQuote(await api<SignedQuote>("/api/quote", { method: "POST", body: JSON.stringify({ id: agent.id, task }) })); } catch (e) { setError((e as Error).message); } finally { lock.current = false; setBusy(""); } }
  async function activate() {
    if (!wallet || !client) { openWallet(); return; }
    if (!quote || !approved || lock.current || record) return;
