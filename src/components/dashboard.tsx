...
type Job = { actions: { approve: boolean; dispute: boolean; refund: boolean }; disputeDeadline: string | null; id: string; client: string; provider: string; description: string; statusName: string; budget: string; expiredAt: string; submittedAt: string; deliverableContent: string | null; deliverableStatus: string; observedAt: string };
export function Dashboard() {
  ...
  function exportHistory() { const blob = new Blob([JSON.stringify(mine, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = "agentmarket-public-activity.json"; link.click(); URL.revokeObjectURL(url); }
