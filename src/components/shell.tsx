...
const LAST_ACCOUNT_KEY = "binera-last-passkey-account-v1";
const AUTO_RESTORE_DISABLED_KEY = "binera-passkey-auto-restore-disabled-v1";
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const preferred = accounts.find(account => account.credential.id === lastId) ?? accounts[0];
      queueMicrotask(() => { if (!cancelled) { setSavedAccounts(accounts); setSelectedAccount(preferred?.credential.id || ""); } });
      if (preferred && localStorage.getItem(AUTO_RESTORE_DISABLED_KEY) !== "true") {
        void import("@altananetwork/sdk").then(sdk => {
          if (cancelled) return;
          const c = sdk.createClient({ chains: [sdk.BNB] });
          const w = { address: preferred.address, signer: sdk.signerFromPasskey(preferred.credential) };
          setClient(c); setWallet(w);
        }).catch(() => { /* The saved reference remains available for manual unlock. */ });
      }
    } catch { queueMicrotask(() => { if (!cancelled) setError("Saved account references could not be read. Keep browser data and any account backup; do not create a replacement account to recover funds."); }); }
  ...
  async function connect(mode: "create" | "recover" | "resume") {
    ...
    try {
      ...
      if (w.signer.credential.kind === "webauthn") {
        ...
        localStorage.setItem(LAST_ACCOUNT_KEY, w.signer.credential.id);
  ...
  return <MarketContext.Provider value={{ compared, toggleCompare, clearCompare: () => updateCompare([]), replaceCompared: updateCompare, wallet, client, openWallet: () => setWalletOpen(true) }}>
    ...
    {walletOpen && <div className="modal-backdrop" onClick={() => !busy && setWalletOpen(false)}><section role="dialog" aria-modal="true" aria-labelledby="account-title" className="modal account-modal" onClick={e => e.stopPropagation()}><button className="icon-button modal-close" aria-label="Close account dialog" disabled={busy} onClick={() => setWalletOpen(false)}><X size={20} /></button><span className="large-icon"><Wallet size={26} /></span><h2 id="account-title">{wallet ? "Your marketplace account" : "An account you control."}</h2><BrowserWalletPanel browser={browser} account={wallet?.address} onBusyChange={setFundingBusy} /><hr />{locationNote}<details className="account-recovery" open={wallet ? undefined : true}><summary>Saved accounts & recovery</summary><p>Binera restores a saved public account session after reload. Your passkey is still required whenever an action needs a signature.</p>{savedAccounts.length > 0 && <><label>Saved marketplace account<select aria-label="Saved marketplace account" disabled={busy} value={selectedAccount} onChange={e => setSelectedAccount(e.target.value)}>{savedAccounts.map(a => <option key={a.credential.id} value={a.credential.id}>{a.address}</option>)}</select></label>{savedBalance && <div className="notice"><div><strong>{savedBalance.native} BNB · {savedBalance.payment} U</strong><p>{savedBalance.keys.length ? `${savedBalance.keys.length} onchain key records loaded.` : "No onchain key registered yet. A saved reference or two-passkey recovery is required."}</p><SourceLink href={`https://bscscan.com/address/${savedAccounts.find(a => a.credential.id === selectedAccount)?.address}`}>Inspect saved account</SourceLink></div></div>}<button className="button primary full" disabled={busy} onClick={() => connect("resume")}>Unlock saved account</button><button className="text-button full" disabled={busy} onClick={exportAccounts}>Export account references</button></>}<label className="button full account-import">Import account references<input className="account-file-input" type="file" accept=".json,application/json" disabled={busy} onChange={e => { void importAccounts(e.target.files?.[0]); e.target.value = ""; }} /></label><small>References contain public metadata, not the passkey. Keep the original passkey in your password manager. Job exports are a different file.</small></details><p>Your marketplace account uses a passkey to approve agent permissions and paid actions.</p>{error && <ErrorNotice message={error} />}{wallet ? <><div className="address-row"><span className="address">{wallet.address}</span><CopyAddress address={wallet.address} label="marketplace account address" /></div><div className="notice">BNB Smart Chain · Fund this account with BNB for network fees and U for agent fees. Send only assets supported on this network.</div><Link className="button primary full" href="/dashboard" onClick={() => setWalletOpen(false)}>Open my agents</Link><button disabled={busy} className="text-button full" onClick={() => { localStorage.setItem(AUTO_RESTORE_DISABLED_KEY, "true"); setWallet(null); setClient(null); setWalletOpen(false); }}>Disconnect this device session</button><small>Disconnecting keeps automatic restore off until you unlock this account again. It does not revoke onchain permissions.</small></> : <><button disabled={busy} className="button primary full" onClick={() => connect("recover")}>{busy ? <LoaderCircle className="spin" size={17} /> : <ShieldCheck size={17} />}Use an existing passkey</button><small>This may request two confirmations to rebuild a missing public account reference.</small><label className="consent"><input type="checkbox" checked={newAccountConfirmed} disabled={busy} onChange={e => setNewAccountConfirmed(e.target.checked)} />I want a separate empty account. This will not recover an existing balance.</label><button disabled={busy || !newAccountConfirmed} className="button full" onClick={() => connect("create")}>Create a passkey account</button><small>No seed phrase. Your passkey stays on your device. A saved account can be unlocked before or after its first transaction. Creating another account keeps existing references.</small></>}</section></div>}
