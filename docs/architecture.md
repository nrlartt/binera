...
Domain: normalized source-agnostic Agent records, four equal categories, explicit unavailable values, deterministic classification and relevance. Registry results are publisher claims, not audited strategies. Registration, endpoint verification and investment risk are separate. Unknown risk must never satisfy a low-risk constraint.
...
Integrations: 8004scan public API (anonymous works), configured BNB Studio A2A cards, Altana SDK on BSC, official PancakeSwap contract reads, optional TermiX MCP read-only tools. No arbitrary registry endpoint may be fetched without SSRF checks. Remote requests have deadlines, bounded bodies and no automatic redirects.
