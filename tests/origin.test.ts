...
test("origin checks support TLS termination without trusting forwarded headers", () => {
  ...
  const extra = process.env.APP_ADDITIONAL_ORIGINS;
  try {
    process.env.APP_ORIGIN = "https://binera-production.up.railway.app";
    process.env.APP_ADDITIONAL_ORIGINS = "https://binera.xyz";
    ...
    assert.doesNotThrow(() => sameOrigin(request("https://binera.xyz")));
    assert.throws(() => sameOrigin(request("https://binera.xyz.attacker.example")), PublicError);
    ...
    if (extra === undefined) delete process.env.APP_ADDITIONAL_ORIGINS;
    else process.env.APP_ADDITIONAL_ORIGINS = extra;
